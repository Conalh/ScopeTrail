...
- Canonical JSON findings now include `salientKey`, populated from ScopeTrail's semantic finding subject. This prevents distinct findings with the same kind and source location from collapsing during fingerprint-based deduplication.

### Migration

- Because `salientKey` participates in fingerprint generation, all ScopeTrail finding fingerprints generated by v0.4.0 differ from their v0.3.x equivalents - not only findings that previously collided.
- Refresh fingerprint-keyed baselines, suppressions, caches, snapshots, and dashboard records after upgrading.
